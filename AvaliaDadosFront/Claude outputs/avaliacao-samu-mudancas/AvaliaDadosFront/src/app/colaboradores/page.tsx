...
import EquipeSupervisorModal from '@/components/modal/EquipeSupervisorModal';
import api, { deleteGlobalCollaboratorApi, updateGlobalCollaboratorEquipeApi } from '@/lib/api';
...
export default function CollaboratorsPage() {
    ...
    const [equipeCollaborator, setEquipeCollaborator] = useState<GlobalCollaborator | undefined>();

    // Candidatos pra compor a equipe de um supervisor: qualquer TARM/Frota/Médico
    // já cadastrado globalmente.
    const equipeCandidatos = useMemo(() =>
        collaborators
    ...
    return (
        <div style={{ margin: '15px' }}>
                                        ...
                                        <TableRow key={collaborator.id}>
                                            ...
                                            <TableCell>
                                                ...
                                                {collaborator.role === 'SUPERVISOR' && (
                                                    <IconButton
                                                        onClick={() => setEquipeCollaborator(collaborator)}
                                                        aria-label={`equipe de ${collaborator.nome}`}
                                                        title="Equipe do supervisor"
                                                        disabled={loading}
                                                    >
            ...
            <EquipeSupervisorModal
                ...
                candidatos={equipeCandidatos}
                escopoDescricao="Esta é a equipe padrão do supervisor, usada como modelo ao criar um novo projeto."
                onSave={async (equipeIds) => {
                    if (!equipeCollaborator?.id) return;
                    await updateGlobalCollaboratorEquipeApi(equipeCollaborator.id, equipeIds);
                    await loadCollaborators();
                }}
            />
        </div>
